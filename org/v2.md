...
* `open` - a base64 string value that is is created by generating a new elliptic (ECC) public key using the nistp256 curve (the key should be in the compressed form), and using RSA to encrypt it (PKCS OAEP padding) *to* the recipients RSA public key.
