...
The string value of the "open" key is created by generating a random secret, and encrypting it *to* the recipients public key, using RSA PKCS1 padding and base64 encoding.  The recipient can then decrypt this using their private key and get the contained secret.  This secret is used to decipher the included binary body using AES-128-CTR, and once unencrypted it is decoded and processed as another packet.

A random 16-byte initialization vector must be generated and used for the AES encryption and encoded as hex in the value of the "iv" key.
