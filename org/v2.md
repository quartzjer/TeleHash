...
* `line` - the unique id the recipient must use for sending any line packets
...
The "type":"connect" is an immediate result of a peer request and must also contain a "connect":"..." with the value being the address (hash,ip,port) of the peer requestor and a BODY of their public key.
