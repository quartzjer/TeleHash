...
* `x` - an integer timestamp (in epoch milliseconds) of the expiration time of this packet (to help avoid reply)

<a name="line" />
#### `line` - Packet Encryption

A packet with a verified `line` value has a BODY that is the fully encrypted original packet.  The receiver then decrypts the BODY and processes it as a normal packet (LENGTH/JSON/BODY) from the sending hashname.  All encryption/decryption uses "AES-128-CBC" and the secret from the sender's original open packet, with the BODY being the binary encoded result.
