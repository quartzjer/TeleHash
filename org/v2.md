...
* Network - A regular hostname representing a collection of hashnames that form a unique single DHT
* Operator - A special hashname that acts as a coordinator to introduce/describe a network to any other hashname (seed)
...
Any network may have special "operators" who help seed new hashnames into it's DHT and can also provide app-specific administrative services. The list of operators is either dynamically resolved via DNS SRV records or simply bundled privately within an app (seed list).  To resolve a list of operators the names returned from a SRV query on _telehash._udp.network must provide one or more hostnames that each individually resolve with an A record their own IP address and a TXT record of their public key in PEM format. A switch trying to connect to that new network without a seed list then has the three required parts to connect to an operator, it's public key, IP, and port.
...
Every network is also a basic Kademlia-based DHT. The bulk of the complexity is in the rules around maintaining a mesh of lines and calculating distance (to be defined again here based on v1). The `"seek":"15f4af33b7edcfbc716cd733a9ed73cc3033c17f"` value is always another hashname that the app is trying to connect to.
