...
(TODO: broken-out examples of how to create and use these links)
...
All `q` packets are minimally required to have a `hash` field and a SHA1 hash as the value (as determined by the type of question it is). That value is the one required to be returned in the `a` so that it can be matched to the original `q`.
