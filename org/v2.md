...
With the decrypted and verified information, the recipient can now also return an open packet (if it hasn't already sent one) and use Diffie-Hellman to derive a shared secret from the elliptic key it generated for this line along with the public elliptic key it received.  The first 16 bytes of this are used as the shared secret for the AES encryption used on all line packets.
...
The BODY is a binary encoded encrypted packet using AES-128-CTR with the shared secret derived from the sender's original open packet and the 16-byte initialization vector decoded from the included "iv" hex value.  Once decrypted, the recipient then processes it as a normal packet (LENGTH/JSON/BODY) from the sending hashname.  All decrypted packets must contain a `"stream":"2588f32b66ac88cb4d02a78d62d1671814b18b82"` value as defined below.
