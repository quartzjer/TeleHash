...
* `open` - a base64 string value that is is created by generating a new elliptic (ECC) public key using the P-256 ([nistp256/secp256r](http://tools.ietf.org/html/rfc6239#page-4)) curve (the key should be in the uncompressed form, 65 bytes), and using RSA to encrypt it (PKCS OAEP padding) *to* the recipients RSA public key.
