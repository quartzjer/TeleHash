...
The required values of the open packet are defined as:
   ...
   * `iv` - 16 random bytes hex encoded to be used as an initialization
     vector
   * `sig` - a base64 string created by first using the sender's RSA public key
     to sign (SHA256 hash and PKCS1 v1.5 padding) the attached
     (encrypted) binary body, and then encrypt that signature using the AES cipher created for the BODY.
...
A rough order of the steps needed create a new open packet are:
  ...
  6. Encrypt the inner packet using the hashed public elliptic key from
     #4 and the IV you generated at #2 using AES-256-CTR (counter 0).
  7. Create a signature from the encrypted inner packet using your own
     RSA keypair, a SHA 256 digest, and PKCSv1.5 padding
	8. Encrypt the signature using the same AES-256-CTR cipher from #6 (counter 1) and base64 encode the result as the value for the `sig` param.
...
An example of the generation of an open packet in Node.js is:
 ...
 open.body = aesCipher.update(attachedRaw);
 open.js.iv = aesIV.toString("hex");
 open.js.sig = aesCipher.update(rsa(myRSAPrivateKey).hashAndSign("sha256", open.body, "PKCS1_PADDING")).toString("base64");
...
rough order of steps:
  ...
  3. Decrypt the inner packet using the generated key and IV value with
     the AES-256-CTR algorithm (counter 0), and decrypt the `sig` value (counter 1).
  ...
  8. Using the RSA public key of the sender, verify the signature (decrypted in #3) of
...
var attached = packetDecode(aesDecipher.update(packet.body));
var sig = aesDecipher.update(new Buffer(packet.js.sig, "base64"));
